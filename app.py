...
import time # 코드 맨 위에 import 되어 있는지 확인하세요


# --- 데이터 로드 (nan 오류 해결 및 안정화 버전) ---
@st.cache_data(ttl=14400)
def load_v45_data():
    # 지수 대신 데이터가 안정적인 ETF 티커로 변경
    tickers = {
        "SP500": "SPY",      # ^GSPC 대신 SPY 사용
        "NASDAQ": "QQQ",     # ^IXIC 대신 QQQ 사용
        "SOXX": "SOXX", "SCHD": "SCHD",
...
help_bb = "미국 개인투자자협회(AAII) 낙관론(Bull)과 비관론(Bear)의 차이 값입니다.\n• 5점(광기): +30% 이상 | 4점(과열): +20% ~ +30% | 3점(중립): -20% ~ +20% | 2점(불안): -30% ~ -20% | 1점(공포): -30% 미만"
...
help_hy = "부실 기업 채권 금리와 국채 금리의 차이입니다.\n• 5점(광기): 3% 이하 | 4점(과열): 박스권 하단(3% ~ 3.5%) | 3점(중립): 3% ~ 5% 박스권 | 2점(불안): 5% 돌파 상승세 | 1점(공포): 5% 이상 폭등 후 하락 전환(진바닥)"
